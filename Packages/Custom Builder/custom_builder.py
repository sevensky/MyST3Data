...
class CustomBuilderPromptCommand(sublime_plugin.WindowCommand):
    ...
    def _select_command(self):
        ...
        # self.window.show_quick_panel([[cmd["title"], cmd["command"]] for cmd in self._commands] + [["<Edit>", "edit command"]],
                                     # self._on_select_command_done2)
    ...
    def _on_select_command_done2(self, index):
        if index == len(self._commands):
            # Ask for a new build command
            self._input_command()
        elif index >= 0:
