...
class StatusInterface(ui.Interface, GitCommand):
    ...
    {< stashes}
      ...
      [s] stage暂存 file                    [A] stage all unstaged and untracked files
      [u] unstage file                      [U] unstage all staged files
      [d] discard changes to file           [D] discard丢弃 all unstaged changes
      [h] open file on remote
      [M] launch external merge tool for conflict 启动外部工具合并冲突
      ...
      [c] commit                            [t][a] apply stash 应用隐藏
      [C] commit, including unstaged        [t][p] pop stash
      [m] amend previous commit修订先前提交 [t][s] show stash
      [P] push current branch推送当前分支   [t][c] create stash
                                            [t][u] create stash including untracked files
      [i] ignore file 忽略                  [t][d] discard stash
      [I] ignore pattern 忽略格式
