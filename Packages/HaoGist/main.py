...
class ChooseGist(sublime_plugin.WindowCommand):
    ...
    def on_done(self, index):
        ...
        # print (index)
        # print (self.items)
        # print (self.items[index])#gist
        # print ("self.items_property[self.items[index]]=%s"  % ( self.items_property[self.items[index]] ) )
...
class AddFileToGist(sublime_plugin.TextCommand):
    def run(self, edit, options={}):
        ...
        sublime.active_window().show_input_panel(message, 
            os.path.split(self.view.file_name())[1] , self.on_input_name, None, None)
