...
class AnywhereCommand(sublime_plugin.TextCommand):
        def run(self, edit):
...
class Anywhere2Command(sublime_plugin.WindowCommand):
        def __init__(self, window):
            """Initialize a new instance."""
            super().__init__(window)
        def run(self):
            # print (self.window.active_view().sel() )
            # print (self.window.active_view().substr(self.window.active_view().sel()[0]))
            pass
...
class SearchAnywhereFromSelectionAskCommand(AnywhereCommand):
    def run(self, edit):
...
class SearchAnywhereFromInputAskCommand(Anywhere2Command):
    def run(self):
...
class SearchAnywhereFromSelectionCommand(AnywhereCommand):
    def run(self, edit):
        for selection in self.view.sel():
            # if the user didn't select anything, search the currently highlighted word
            if selection.empty():
...
class SearchAnywhereFromInputCommand( Anywhere2Command ):
    def run(self):
        super().run()
        settings = sublime.load_settings(__name__ + '.sublime-settings')
